...
export default function MessagesLayout() {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex relative">
      ...
      <div className={`absolute lg:relative w-full lg:w-80 border-r bg-white transition-transform duration-200 ease-in-out ${
        showList ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      }`}>
        <ConversationList onNewMessage={() => setShowNewMessage(true)} />
      </div>

      <div className={`absolute lg:relative inset-0 lg:inset-auto flex-1 bg-gray-50 transition-transform duration-200 ease-in-out ${
