...
export default function ChannelMembers({ channelId, onClose }: ChannelMembersProps) {
    ...
    try {
      ...
      if (classData) {
        ...
        setAvailableUsers(
          (users || []).map(user => ({
            id: user.id,
            label: `${user.owner?.name || user.teacher?.name || user.parent?.name || 'Unknown'} (${user.email})`
          }))
        );
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-brand-primary">Channel Members</h2>
